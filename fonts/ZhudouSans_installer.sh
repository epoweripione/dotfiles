...
INSTALLER_APP_NAME="煮豆黑体 Zhudou Sans"
INSTALLER_GITHUB_REPO="Buernia/Zhudou-Sans"
INSTALLER_INSTALL_NAME="ZhudouSans"

INSTALLER_MATCH_PATTERN="ZhudouSans*"
...
if [[ "${INSTALLER_IS_INSTALL}" == "yes" ]]; then
    ...
    INSTALLER_DOWNLOAD_URLS=(
        "https://github.com/${INSTALLER_GITHUB_REPO}/releases/download/v${INSTALLER_VER_REMOTE}/Zhudou.Sans.zip"
    )
    ...
    if [[ ${INSTALLER_DOWNLOAD_COUNT} -eq ${#INSTALLER_DOWNLOAD_URLS[@]} ]]; then
        ...
        # Install fonts
        install_fonts=$(find "${WORKDIR}" -type f -iname "${INSTALLER_MATCH_PATTERN}" -a \( -name "*.ttf" -o -name "*.ttc" \) ! -name "*VF*")
