...
# [阿里健康体](https://www.alibabahealthdesign.com/)
INSTALLER_APP_NAME="阿里健康体"
INSTALLER_INSTALL_NAME="AlibabaHealthDesign"

INSTALLER_MATCH_PATTERN="AlibabaHealthFont*"
...
if [[ "${INSTALLER_IS_INSTALL}" == "yes" ]]; then
    ...
    INSTALLER_CHECK_URL="https://www.alibabahealthdesign.com/"
    INSTALLER_VER_REMOTE=$(curl "${CURL_CHECK_OPTS[@]}" "${INSTALLER_CHECK_URL}" 2>/dev/null \
        | grep -Eo -m1 "阿里健康体([0-9]{1,}\.)+[0-9]{1,}" \
...
if [[ "${INSTALLER_IS_INSTALL}" == "yes" ]]; then
    ...
    INSTALLER_DOWNLOAD_URLS=(
        "https://alijk-cdn-assets.oss-cn-beijing.aliyuncs.com/%E9%98%BF%E9%87%8C%E5%81%A5%E5%BA%B7%E4%BD%93${INSTALLER_VER_REMOTE}.zip"
    )
