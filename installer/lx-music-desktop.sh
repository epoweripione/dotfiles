...
if [[ "${INSTALLER_IS_INSTALL}" == "yes" ]]; then
    if check_os_arch; then
        if App_Installer_Get_Remote "https://api.github.com/repos/${INSTALLER_GITHUB_REPO}/releases/latest" "lx-music-desktop.*\.pacman"; then
            if App_Installer_Download "${INSTALLER_DOWNLOAD_URL}" "$(xdg-user-dir DOWNLOAD)/lx-music-desktop.pacman"; then
                sudo pacman -U "$(xdg-user-dir DOWNLOAD)/lx-music-desktop.pacman"
