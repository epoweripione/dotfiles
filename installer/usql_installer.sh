...
APP_INSTALL_NAME="usql"
GITHUB_REPO_NAME="xo/usql"

ARCHIVE_EXT="tar.bz2"
ARCHIVE_EXEC_DIR=""
ARCHIVE_EXEC_NAME="usql*"

EXEC_INSTALL_PATH="/usr/local/bin"
EXEC_INSTALL_NAME="usql"
    ...
    case "${OS_INFO_TYPE}" in
        linux)
            case "${OS_INFO_ARCH}" in
                amd64)
                    REMOTE_FILENAME="usql_static-${REMOTE_VERSION}-${OS_INFO_TYPE}-amd64.${ARCHIVE_EXT}"
                    ;;
            esac
            ;;
        darwin)
            REMOTE_FILENAME="usql-${REMOTE_VERSION}-${OS_INFO_TYPE}-amd64.${ARCHIVE_EXT}"
            ;;
    esac
