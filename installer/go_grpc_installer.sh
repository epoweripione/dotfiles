...
APP_INSTALL_NAME="protobuf"
GITHUB_REPO_NAME="protocolbuffers/protobuf"

ARCHIVE_EXT="zip"
ARCHIVE_EXEC_DIR="bin"
ARCHIVE_EXEC_NAME="protoc"

GOPATH=$(go env GOPATH 2>/dev/null)
EXEC_INSTALL_PATH="${GOPATH:-"/usr/local"}/bin"
EXEC_INSTALL_NAME="protoc"
    ...
    case "${OS_INFO_TYPE}" in
        linux)
            case "${OS_INFO_ARCH}" in
                amd64)
                    REMOTE_FILENAME="protoc-${REMOTE_VERSION}-${OS_INFO_TYPE}-x86_64.${ARCHIVE_EXT}"
                    ;;
                386)
                    REMOTE_FILENAME="protoc-${REMOTE_VERSION}-${OS_INFO_TYPE}-x86_32.${ARCHIVE_EXT}"
                    ;;
                arm64)
                    REMOTE_FILENAME="protoc-${REMOTE_VERSION}-${OS_INFO_TYPE}-aarch_64.${ARCHIVE_EXT}"
                    ;;
                ppc64le)
                    REMOTE_FILENAME="protoc-${REMOTE_VERSION}-${OS_INFO_TYPE}-ppcle_64.${ARCHIVE_EXT}"
                    ;;
                s390x)
                    REMOTE_FILENAME="protoc-${REMOTE_VERSION}-${OS_INFO_TYPE}-s390_64.${ARCHIVE_EXT}"
                    ;;
            esac
            ;;
        darwin)
            REMOTE_FILENAME="protoc-${REMOTE_VERSION}-osx-x86_64.${ARCHIVE_EXT}"
            ;;
    esac
...
if [[ "${IS_INSTALL}" == "yes" ]]; then
    ...
    if [[ ${curl_download_status} -eq 0 ]]; then
        ...
        if [[ -s "${ARCHIVE_EXEC_DIR}/${ARCHIVE_EXEC_NAME}" ]]; then
            cp -f "${ARCHIVE_EXEC_DIR}/${ARCHIVE_EXEC_NAME}" "${EXEC_INSTALL_PATH}/${EXEC_INSTALL_NAME}" && \
                chmod +x "${EXEC_INSTALL_PATH}/${EXEC_INSTALL_NAME}" && \
