...
if [[ -f "${INSTALLER_ARCHIVE_FILENAME}" ]]; then
    ...
    sudo tar -xzf "${INSTALLER_ARCHIVE_FILENAME}" -C "${MANUAL_INSTALL_DIR}" && \
        sudo mv "${MANUAL_INSTALL_DIR}/GoldenDict" "${MANUAL_INSTALL_DIR}/GoldenDict++OCR" && \
        sudo chown -R "$(id -u)":"$(id -g)" "${MANUAL_INSTALL_DIR}/GoldenDict++OCR" && \
        sudo find "${MANUAL_INSTALL_DIR}/GoldenDict++OCR" -type d -print0 | xargs -0 chmod 755 && \
        sudo chmod +x "${MANUAL_INSTALL_DIR}/GoldenDict++OCR/goldendict.sh" && \
        sudo chmod +x "${MANUAL_INSTALL_DIR}/GoldenDict++OCR/GoldenDict"
fi

if [[ -f "${MANUAL_INSTALL_DIR}/GoldenDict++OCR/splash.png" && ! -f "/usr/share/pixmaps/GoldenDict++OCR.png" ]]; then
    sudo cp "${MANUAL_INSTALL_DIR}/GoldenDict++OCR/splash.png" "/usr/share/pixmaps/GoldenDict++OCR.png"
fi

if [[ -f "${MANUAL_INSTALL_DIR}/GoldenDict++OCR/goldendict.sh" && ! -f "${DESKTOP_DIR}/GoldenDict++OCR.desktop" ]]; then
...
Exec=${MANUAL_INSTALL_DIR}/GoldenDict++OCR/goldendict.sh
Icon=GoldenDict++OCR
Path=${MANUAL_INSTALL_DIR}/GoldenDict++OCR
Terminal=false
StartupNotify=true
...
## Run `GoldenDict++OCR`
# if [[ -f "${MANUAL_INSTALL_DIR}/GoldenDict++OCR/goldendict.sh" ]]; then
