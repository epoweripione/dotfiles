...
# $env:_PSFZF_FZF_DEFAULT_OPTS = "--height=40% --layout=reverse --preview-window=hidden"
$env:_PSFZF_FZF_DEFAULT_OPTS = "--height=50% --min-height=8 --layout=reverse --info=right --scrollbar='▐'"
$PSFZF_CONFIG = "$env:USERPROFILE\Documents\PowerShell\Scripts\PSfzf-config.ps1"
if ((Test-Path "$PSFZF_CONFIG") -and ((Get-Item "$PSFZF_CONFIG").length -gt 0)) {
    . "$PSFZF_CONFIG"
    Register-SmartPsFzfHandlers -EnableLogging $false
}
##-------------------------------------------PSFzf: fix the shell prompt offset issue-------------------------------------------
...
# [Intelligent context-aware command completion with Az Predictor](https://learn.microsoft.com/en-us/powershell/azure/az-predictor)
